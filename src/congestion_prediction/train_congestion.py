...
"""
Training script for the congestion prediction model.  
"""
...
def save_model(model: nn.Module, optimizer: optim.Optimizer, epoch: int, val_loss: float, config: Dict) -> None:
    model_name = f"mapf_congestion_model_hidden{config['hidden_channels']}_lr{config['lr']}_epoch{epoch+1}_valMSE{val_loss:.4f}.pth"
    torch.save({
        'epoch': epoch,
